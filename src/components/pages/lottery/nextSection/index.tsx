...
const NextSection: React.FC<Props> = ({nextData, playerData, sendDataNextToLottery}) => {
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(function() {
      ...
      if (days > 0) {
        timerData = days + "<span class='uppercase text-40 md:text-32'>d</span> " + hours + "<span class='uppercase text-40 md:text-32'>h</span>" + minutes + "<span class='uppercase text-40 md:text-32'>m</span> " + seconds + "<span class='uppercase text-40 md:text-32'>s</span> ";
      } else {
        timerData = hours + "<span class='uppercase text-40 md:text-32'>h</span> " + minutes + "<span class='uppercase text-40 md:text-32'>m</span> " + seconds + "<span class='uppercase text-40 md:text-32'>s</span> ";
      }
            ...
            nextData.next_id > 0 ? (
              <>
                <div className='p-3.5 md:px-8 md:pt-4 md:pb-8'>
                  <p className='text-14 md:text-16 font-bold mb-2 md:mb-3'>Next Party</p>
                  <p className='bg-gray-F9F9F9-20 text-right rounded-5 flex justify-between items-center text-gray-A9A9A9 pr-2.5'>
                    <span className='text-14 md:text-16 font-semibold text-pink-F4E0FF p-2 md:px-8 text-center bg-pink-8C24BF rounded-5 inline-block'>#{nextData.next_id}</span>
                    {`${monthName[nextData.closed_time.getUTCMonth()]} ${nextData.closed_time.getUTCDate()}, 
                      ${nextData.closed_time.getFullYear()}, 
                      11:00 AM UTC`}
                  </p>
                </div>
                <p className='bg-gray-575757-50 h-px'></p>
                <div className='px-3.5 py-1.5 md:px-8 md:pt-6 md:pb-5 flex items-end justify-between'>
                  <div>
                    <p className='text-14 md:text-16 font-bold md:mb-1'>Your Ticket</p>
                    <p className='text-gray-EBEBEB'>You have <span onClick={handleViewTicket} className='text-blue-ADFAFF underline cursor-pointer hover:no-underline inline-block font-bold'>{nextData.your_tickets.length} ticket</span> to enter this party.</p>
                  </div>
                  <div className='relative w-92 md:w-115'>
                    <p className={`bg-blue-17F0FF flex items-center justify-center h-32px md:h-34px font-semibold text-blue-0B7880 rounded-5 ${playerData.is_connect ? 'cursor-pointer hover:opacity-75' : 'cursor-not-allowed'}`}
